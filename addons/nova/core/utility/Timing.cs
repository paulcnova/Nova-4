...
namespace Nova
{
	/// <summary>A class that creates coroutines.</summary>
	public partial class Timing : Node
	{
		...
		/// <summary>A constructor for the timing node.</summary>
		public Timing()
		{
			InitializeInstanceID();
		}
		
		/// <summary>Gets the instance for the timing node</summary>
		public static Timing Instance
		{
			get
			{
				if (_instance == null)
				{
					// Check if we were loaded via Auto-Load
		...
		{
			var waitTriggersEnum = _waitingTriggers.GetEnumerator();
			while (waitTriggersEnum.MoveNext())
			{
				if (waitTriggersEnum.Current.Value.Count == 0)
				{
					_waitingTriggers.Remove(waitTriggersEnum.Current.Key);
					waitTriggersEnum = _waitingTriggers.GetEnumerator();
					continue;
				}
				
				if (_handleToIndex.ContainsKey(waitTriggersEnum.Current.Key) && CoIndexIsNull(_handleToIndex[waitTriggersEnum.Current.Key]))
				{
					CloseWaitingProcess(waitTriggersEnum.Current.Key);
					waitTriggersEnum = _waitingTriggers.GetEnumerator();
				}
			}
		...
		/// no effect.
		/// </summary>
		/// <param name="tag">Any coroutines previously paused with a matching tag will be resumed.</param>
		/// <returns>The number of coroutines that were resumed.</returns>
		...
		private void SwapToLast(CoroutineHandle firstHandle, CoroutineHandle lastHandle)
		{
			...
			{
				var triggersEnum = _waitingTriggers[lastHandle].GetEnumerator();
				while (triggersEnum.MoveNext())
					SwapToLast(lastHandle, triggersEnum.Current);
			}
...
public static class MECExtensionMethods2
{
	...
	{
		while (Nova.Timing.MainThread != System.Threading.Thread.CurrentThread || (IsNodeAlive(node) && coroutine.MoveNext()))
	...
	{
		while (Nova.Timing.MainThread != System.Threading.Thread.CurrentThread || (IsNodeAlive(node1) && IsNodeAlive(node2) && coroutine.MoveNext()))
			yield return coroutine.Current;
	}
	...
	{
		while (Nova.Timing.MainThread != System.Threading.Thread.CurrentThread || (IsNodeAlive(node1) && IsNodeAlive(node2) && IsNodeAlive(node3) && coroutine.MoveNext()))
			yield return coroutine.Current;
	}
}
